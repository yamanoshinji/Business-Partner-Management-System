...
**主仕様書**: `docs/spec/product-spec.md` を必ず参照すること。  
**補足**: `docs/spec/mvp-spec.md` は MVP 完了時点のアーカイブとして扱うこと。
...
**機能追加フェーズでの扱い**:
- ユーザーの指示に基づく機能追加は対象としてよい。
- 新機能は `docs/spec/product-spec.md` と矛盾しないように実装すること。
- 影響の大きい仕様変更は、実装前または実装と同時に仕様書へ反映すること。
- 次の変更は要件を明文化してから扱うこと。
  - 認証方式や権限管理の変更
  - 外部通知チャネルの追加
  - 外部システム連携
...
├── docs/
│   └── spec/
│       ├── product-spec.md # 現行仕様書（常に参照すること）
│       └── mvp-spec.md     # MVP 完了時点のアーカイブ
...
- 仕様や要件の整理なしに影響の大きい機能追加を独断で進めること
